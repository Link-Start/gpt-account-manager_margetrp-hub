# 更新记录

## 1.3.0

- 新增工作区找回页 `/recover.html`，支持当前浏览器工作区恢复、管理员查看服务器工作区摘要，以及本地导出 / 导入备份。
- 备份恢复支持可选密码加密，恢复时会还原账号、分组、队列和常用设置，但不会导出管理员工具 token。
- 修复新用户共享硬编码默认工作区的问题，避免不同浏览器实例互相看到同一个默认空间。
- 更新静态资源版本号到 `1.3.0`，让前端脚本自动换新。
- 本地回归测试保持通过：`92 passed`。

## 1.2.0

- 修复线上邮箱管理页初始化顺序问题：先创建邮箱账号模型，再读取当前工作区邮箱资产，避免页面显示 0、按钮看起来失效。
- 修复凭证刷新页长效手机池初始化顺序问题：手机池 helper 变量提前进入作用域，刷新队列不再因为前端脚本报错停在空状态。
- 版本号提升到 `1.2.0`，让线上静态资源 URL 自动换新，减少浏览器继续缓存旧 `mailboxes.js` / `refresh.js` 的概率。

## 1.1.1

- 凭证刷新页继续完成前端分层整理：新增 `refresh-source-list.js`、`refresh-manual-code.js`、`refresh-phone-pool.js` 和 `refresh-queue-model.js`，把来源列表、邮箱验证、手动验证码弹窗、长效手机池和本地刷新队列拼装从主文件里抽出来，`refresh.js` 从 4300+ 行收薄到更容易继续维护的体量。
- 邮箱管理页新增“加入刷新队列”批量入口，真正按当前勾选邮箱把账号推进刷新队列；首页、邮箱管理页和刷新页共用同一套本地队列模型，减少三个页面各自维护一套入队逻辑带来的回归风险。
- 首页和刷新页继续收紧界面交互：来源列表、手动邮箱码/手机码、长效手机绑定与拉码、删除刷新队列但保留邮箱库等关键行为统一走共享 helper，主文件更多只保留桥接和事件绑定。
- 这一版重点不是再加新页面，而是把已有刷新链路继续拆稳，给后续继续拆 `refresh-runner / job-polling / login-payload` 和整理 `styles.css` 打好基础。
- 当前这一轮整理后，回归测试仍然保持 `89 passed`。

## 1.1.0

- 并行完成三条核心基础设施升级：A1 仪表盘/活动统计 SQLite 查询层、A2 登录任务状态 SQLite 持久化、A3 邮件缓存 SQLite 化，项目从“JSON 为主、SQLite 试探接入”推进到“活动与邮件缓存已有稳定 SQLite 主路径”。
- `dashboard_stats.py` 新增面向活动路径的 SQLite 优先统计入口，`refresh_results / login_history` 支持可复用的 SQLite 查询接口，同时保留旧 JSON 兼容兜底和自动回填。
- 新增 `storage/login_job_store.py`，把登录任务关键状态、日志、手动邮箱码/手机码、终止状态和重启恢复写入 SQLite；服务重启后会恢复队列/历史视图，未完成任务统一标记为 `login_interrupted`，避免前端无限轮询假运行状态。
- 新增 `storage/message_sqlite_store.py`，邮件缓存现在支持 SQLite sidecar，服务端读取邮件缓存优先走 SQLite；如果只有旧 `messages.json`，首次读取会自动回填到 SQLite，同时继续兼容现有前端接口和 `public` 工作区 root/workspace 双路径缓存。
- 补齐 `tests/test_dashboard_stats.py`、`tests/test_login_job_store.py`、`tests/test_login_job_persistence.py`、`tests/test_message_store.py` 等回归覆盖，当前这一轮合并后的总测试结果为 `89 passed`。

## 1.0.8

- 活动数据读取正式切到 SQLite 优先：`refresh_results` 和 `login_history` 现在会先读 `.sqlite3` sidecar，没有 sidecar 时再回退到原有 JSON 快照。
- 旧 JSON 活动数据第一次被读取时会自动回填到 SQLite sidecar，不需要额外的手工迁移步骤；这样现有部署升级后，活动数据会自然完成“边读边迁”。
- 扩展 `tests/test_activity_store.py`，补齐旧 JSON 回填 SQLite 的覆盖，确保这次不是只做双写，而是真正把活动读取主路径切到了更稳的持久化层。

## 1.0.7

- 开始把活动数据从纯 JSON 向更稳的持久化层推进：新增 `storage/activity_sqlite_store.py`，为刷新结果和登录历史生成 SQLite sidecar。
- `storage/activity_store.py` 现在在保持原有 JSON 快照兼容的同时，会把刷新结果和登录历史双写到对应的 `.sqlite3` 文件里，为后续把活动查询和统计逐步切到 SQLite 做准备。
- 新增 `tests/test_activity_store.py`，覆盖刷新结果和登录历史的快照写入、追加更新以及 SQLite sidecar 建表和去重行为，确保这次落地是“先稳接口，再迁存储”。

## 1.0.6

- 继续完成 `server.py` 的 HTTP 分发拆层，新增总的 `http_handlers.py`，把公共 GET、管理员 GET、client API、workspace API、认证 POST 和管理员 POST 统一移入独立分发模块。
- `server.py` 里的 `Handler` 现在主要保留 `BaseHTTPRequestHandler` 外壳、鉴权辅助、工作区解析、静态文件服务和 JSON 输出底层方法，通用路由不再继续堆在一个巨型 `if/elif` 里。
- 新增 `tests/test_http_handlers.py`，覆盖 public-config、client accounts、workspace messages、auth login、client fetch、workspace import 和静态页兜底，保证完整分发层拆完后主路径仍然稳定。

## 1.0.5

- 继续拆分 `server.py`，把 CPA 仓管和凭证刷新相关的客户端 HTTP 路由独立到新的 `cpa_http_handlers.py`，包括登录状态查询、生命周期刷新、401 扫描/修复、auth 替换、手动填码、任务终止和已停用 OAuth 路径提示。
- `Handler` 里的 CPA/刷新路由分支改为调用独立 handler 模块，`server.py` 继续收缩成入口层，后面再拆 `http_handlers` 其他部分时能沿着同样的模式继续推进。
- 新增 `tests/test_cpa_http_handlers.py`，覆盖 CPA 登录状态、生命周期刷新、登录启动参数补丁、登录异常分类和已停用 OAuth 接口，保证路由抽离后返回结构保持稳定。

## 1.0.4

- 继续拆分 `server.py`，把凭证生命周期刷新逻辑独立到新的 `refresh_lifecycle_service.py`：统一接管 RT / session_token / access_token 三条刷新与探测路径，减少 `server.py` 和 `cpa_client.py` 之间的隐式耦合。
- `server.py` 保留兼容旧调用的薄包装入口，`cpa_client.py` 改为直接依赖新的生命周期 service 方法，后面继续拆 `http_handlers`、刷新预算和错误聚合时，不需要再穿回主文件里找一堆 helper。
- 新增 `tests/test_refresh_lifecycle_service.py`，覆盖空请求、RT 成功刷新、封禁 RT 分类、access_token 探测和汇总计数，保证这次抽离有回归保护。

## 1.0.3

- 继续推进 `server.py` 分层，这一轮先把 CPA 远端交互和仓管操作独立成新的 `cpa_client.py`，把 OAuth 授权入口、auth file 上传/下载、401 巡检、批量修复、删除和生命周期刷新从主文件里抽出来。
- `server.py` 里的 CPA 路由现在只保留薄入口，旧的 CPA helper 已清理掉，后面继续拆 `refresh_state_machine`、`http_handlers` 和更多 provider 时，边界会更清楚。
- 新增 `tests/test_cpa_client.py`，直接覆盖 CPA OAuth 启动、auth 替换探测和 401 生命周期筛选，保证这次分层不是只挪代码、没有回归保护。

## 1.0.2

- 继续收紧工作区级数据隔离：工作区邮件查询、已保存邮箱收信和收信结果落盘统一走新的消息查询服务与收信服务封装，减少 `server.py` 里的重复筛选和状态写回逻辑。
- 修复邮箱列表与凭证刷新队列在恢复轮询、重新入队和手动单个执行时的边界行为，避免任务状态刷新把不可见筛选项、旧轮询状态或重复日志重新带回来。
- 提升前端同步稳定性：邮箱同步、收信状态轮询增加超时保护和更细粒度的局部刷新，降低页面长时间挂起时整块 UI 一起抖动的概率。

## 1.0.1

- 修复首页邮箱清单“全选后再搜索”时的收取范围判断：当前如果已经通过搜索、来源或分类把可见邮箱缩小，点击“收取邮件”会优先只处理当前可见且被勾选的邮箱，不再把隐藏掉的旧选中邮箱一起带进去。
- 静态资源版本提升到 `1.0.1`，避免浏览器继续缓存 `1.0.0` 的旧 `client.js`，导致收取范围修复不生效。

## 1.0.0

- 版本号正式提升到 `1.0.0`，把这一阶段已经稳定下来的账号管理、邮箱收信、凭证刷新、CPA 仓管、仪表盘和 Docker 部署能力收成首个里程碑版本。
- 收信链路继续按来源拆开处理：Outlook / Microsoft、临时邮箱 JWT、其他 IMAP / POP3 邮箱分别走各自取信逻辑，同时统一错误分类、导入入口和页面命名。
- 邮件缓存改为服务端工作区存储，首页、邮箱管理、刷新页和仪表盘围绕同一份工作区数据协作，减少浏览器 `localStorage` 超额和多用户串数据风险。
- 凭证刷新保持协议状态机主链路，补齐邮箱验证码、手机验证码、失败分类、手动填码、终止任务和队列执行等关键交互，便于把错误收敛到可处理类型。
- 页面结构完成一轮整理：账号管理、邮箱管理、凭证刷新、CPA 仓管、Session 转换、部署自检和风险仪表盘都已有独立入口，适合继续按模块迭代。
- 部署方式以 Docker Compose 为主线，补齐迁移脚本、升级 agent、自检页面和版本化静态资源，方便 VPS 持久化部署、同步升级和回滚排查。

## 0.8.5

- 第一阶段拆分 `server.py`：新增 `storage/workspace.py`，把工作区 ID 归一化、工作区文件路径、JSON 读写和数据计数集中到数据隔离层，降低邮箱库和刷新队列互相影响的风险。
- 新增 `mail_providers.py`，统一维护其他邮箱主机推断、收信模式归一化、Microsoft provider 顺序、收信错误分类和并发收信调度；`server.py` 只保留具体取信实现和薄包装。
- 新增 `refresh_state_machine.py`，先把刷新任务的细阶段拆成 `checking_mail / waiting_code / building_auth / syncing_cpa / success / failed` 等状态，同时继续兼容前端使用的 `queued / running / success / failed`。
- 清理 `server.py` 里重复的邮箱 provider 常量、错误标签和并发调度代码，为后续继续拆 `cpa_client`、`dashboard_stats` 和 `http_handlers` 留出清晰边界。

## 0.8.4

- 修复导入弹窗按邮箱服务显示错误示例的问题，临时邮箱模式不再出现 Outlook 四段格式。
- “其他邮箱”入口补充 163、QQ、iCloud、Gmail 等 IMAP/POP3 示例，避免用户误以为只支持 Outlook 和临时邮箱。
- 首页、邮箱管理页、刷新页和仪表盘统一使用“其他邮箱”命名，并修复动态导入提示被语言切换缓存回滚的风险。
- README 补充账号管理、风险仪表盘、CPA 仓管、Session 转换和部署自检等新增页面截图，并加入后续优化路线图。

## 0.8.3

- 修复首页左侧邮箱清单标题区统计重叠，改为两列来源筛选按钮。
- 邮箱清单支持按全部、Outlook、临时、其他来源点击筛选，其他会兜底普通邮箱和未知来源邮箱。
- 切换来源筛选时自动清理隐藏行的勾选状态，避免批量操作误带不可见邮箱。

## 0.8.2

- 版本号切换为语义化数字版本，后端健康检查、`package.json` 和静态资源缓存串统一使用 `0.8.2`。
- 首页收信固定走工作区隔离的 `/client-api/fetch-start` 后台任务，不再因打码凭证回退到全局 `/api/fetch`。
- CPA 仓管页移除手动 Session JSON 兜底入口，401 修复统一加入凭证刷新队列执行协议登录。
- 清理已停用的手动 OAuth 包装函数和前端旧样式，保留旧接口的明确停用响应，方便老页面得到可读错误。

## 2026-06-03 dashboard-insights

- 仪表盘新增风险摘要、缓存邮件趋势、邮件分类分布、取信来源分布、邮箱状态和邮箱错误类型统计。
- `/client-api/dashboard-stats` 增加风险级别、最新缓存时间和邮件流量统计，继续保持只读，不触发取信或删除。
- 优化仪表盘响应式布局和中英文文案，并统一静态资源版本避免浏览器缓存旧脚本。

## 2026-06-03 dashboard-stats

- 新增工作区风险仪表盘，汇总封禁邮件趋势、今日/近 7 天/周期封禁数量、涉及邮箱、域名排行和最近封禁邮件。
- 新增 `/client-api/dashboard-stats` 只读接口，基于当前工作区缓存邮件、邮箱资产和刷新结果生成统计，不触发取信或删除。
- 顶部模块导航新增“仪表盘”入口，并补充中英文切换文案。

## 2026-06-03 generic-mail-provider

- 新增普通邮箱取信入口，支持 IMAP/POP3 自动推断以及 CloudMail/LuckMail/Inbucket 类 API 取信。
- 邮箱管理页支持普通邮箱导入、同步、筛选、导出和删除。
- 凭证刷新页的邮箱预检和登录取码链路支持普通邮箱，并补充对应中文错误分类。

## 2026-06-01 server-mail-cache

- 邮件正文和 HTML 改为写入服务端工作区缓存 `data/workspaces/<workspace>/messages.json`，浏览器不再保存完整邮件数组。
- 首页邮件列表改成从 `/client-api/messages` 分页读取，筛选、分页、单封删除、批量删除都直接操作服务端缓存。
- 收信接口只返回轻量结果：邮件数量、验证码列表和首个验证码，避免大批邮件触发浏览器 `localStorage` 配额错误。
- CPA 仓管页“收取验证码”不再写 `ctgptm.mail.messages`，只使用轻量验证码摘要。
- 刷新页邮箱验证改为使用 `codes / first_code / has_verification_code / message_count`，不再依赖完整邮件正文返回。

## 2026-06-01 fetch-background

- 邮箱读取改为后台任务：前端只启动任务并轮询状态，后端继续直接连接 Outlook / 临时邮箱 API 收信，避免网页长请求被 Nginx/Cloudflare 504 截断。
- 撤销整批 45 秒截止线；大量邮箱会按并发池继续跑完，不会把还没排到的邮箱统一标成取信超时。
- 前端遇到 504/HTML 网关错误时显示可读提示，不再把整页错误 HTML 当作业务错误展开。

## 2026-06-01 outlook-scroll

- 修复首页左侧邮箱清单固定高度下无法滚动的问题，选择 20 / 50 条时不再裁掉后续邮箱。
- Outlook 自动收信优先走 IMAP，并保留 Graph / Outlook API 作为回退；Graph 401 不再把 IMAP 可用账号误标为失败。
- IMAP token 刷新顺序改为优先 `login.live.com`，兼容更多 Outlook 四段凭证。
- 前端收信方式统一规范为 `auto / graph / imap`，选择单一路径失败时会提示切回微软自动。

## 2026-06-01 final

- 增加轻量国际化脚本，默认中文，右上角可切换中文 / EN。
- 语言选择保存在当前浏览器，本地刷新和页面跳转后保持一致。
- 覆盖顶部导航、主要工作台按钮、筛选项、表头、常见提示和输入框占位文案。
- 国际化层只翻译界面固定文案，不处理用户邮箱、token、代理 URL 或日志里的业务数据。

## 2026-06-01

- 刷新页新增“验证邮箱”按钮，先独立检查取码邮箱是否可收件、当前是否能读到验证码邮件。
- 验证邮箱会自动使用当前工作区保存的真实取码凭证，避免把前端打码后的 Outlook/临时邮箱凭证误判为失败。
- 已验证失败或未验证的邮箱不会直接加入刷新队列，避免把坏邮箱带进 OAuth 刷新流程。
- 邮箱验证结果写回左侧邮箱状态：可收件、未发现验证码、Graph/IMAP 收信失败、临时邮箱 JWT 无效等。
- 邮箱管理页权限类型改为 `JWT`、`Graph/IMAP`、`Graph+IMAP+密码`，导入类型下拉只保留邮箱名称，不再误写成 OAuth。
- README 增加脱敏界面截图，关键邮箱、token、代理信息已打码。
- 邮箱管理页继续打磨：左侧邮箱管家改为浅色高对比，不再被全局玻璃主题冲淡。
- 邮箱管理页移除异常主分类；异常邮箱只在邮箱名旁标红“（错误）”。
- 新增独立“邮箱管理”页面：左侧分类统计，右侧表格管理，支持搜索、来源/分组筛选、分页、批量复制、批量分组、导出备份和批量删除。
- 邮箱管理页支持 Outlook 邮箱和临时邮箱导入；临时邮箱导入新增工作空间级接口，导入后刷新页也能同步到。
- 顶部功能导航增加“邮箱管理”入口，避免把邮箱资产管理继续堆在凭证刷新页里。
- 刷新队列顶部统计改成可点击筛选：队列、等待、执行中、成功、失败都能直接切换列表。
- 失败账号清理改为按“已选账号 / 当前失败筛选 / 全部失败”批量处理，不再只限制封禁、账号不存在等确定失败类型。
- 左侧已导入邮箱增加“移除所选”，可在成功/失败/需要接码筛选后批量移除对应取码资料和队列记录。
- 邮箱验证码、手机验证码都改为按需弹窗输入，默认继续自动收码，不再在页面常驻输入框。
- 正在执行或排队的账号增加“终止”，后端轮询邮箱码/手机码时会响应终止并停止等待。
- 长效手机池进一步压缩为折叠内紧凑布局，手动手机验证码改为弹窗填写。
- 凭证刷新改为队列优先：单个账号按钮只加入队列，统一由刷新页顺序执行，避免手动点击时绕过排队。
- CPA 异常扫描台和仓管页的刷新动作改为“加入刷新队列”，不再在原页面直接启动登录任务。
- 协议登录默认走邮箱验证码路径，避免把 Outlook/临时邮箱的取码凭证误当成 OpenAI 登录密码提交。
- 优化邮箱验证码链路：支持 passwordless 发码接口，按真实收信时间过滤旧验证码，并修正验证码提取误识别英文单词的问题。
- 增加 OAuth 手机二次验证状态机：识别 `phone_otp_select_channel` / `/phone-otp/select-channel`，进入短信通道后等待长效手机 API 或手动手机验证码。
- 长效手机接码池支持批量导入、1 对 1 绑定、按手机号尾号匹配账号，并把手动输入的手机验证码回传给正在运行的登录任务。
- 代理处理增加粘性会话：支持 `{session}` 占位符，并对部分代理服务自动追加 session；同一账号执行前会复检出口 IP 是否稳定。
- 网络错误分类细化为 DNS 失败、代理超时、代理 TLS 中断、代理连接失败、代理出口不稳定等大类，减少“登录失败”这类笼统提示。
- 刷新失败自动重试扩展到可恢复的代理、DNS、风控和 OAuth 步骤异常；确定失败类账号继续保持不可重试。
- sub2api / CPA 导出继续保持成功结果里的 `auth_file` 结构，成功刷新记录保存到工作区 `refresh_results.json`。

## 2026-05-31

- 增加 Dockerfile、docker-compose.yml 和 Docker 部署说明，开源部署可以直接构建容器。
- 增加旧 systemd 部署迁移到 Docker 的脚本，保留原 data 和环境变量，健康检查失败时回滚旧服务。
- Docker Compose 增加显式 DNS；容器健康检查改为 `/network-health`，更早发现凭证刷新所需域名解析失败。
- 顶部商城/中转站/公益站入口改为运行时配置：开源默认隐藏，自用 `.env` 填 URL 后显示。
- 自检页增加“同步升级”入口，配合宿主机 upgrade agent 可点击请求 `git pull + docker compose` 重建重启。
- 刷新页上方配置区改为可展开/收起的小段，压缩代理、CPA、取码邮箱和长效手机池占用空间。
- 刷新页增加“清理确定失败”动作，可把封禁/不存在/地区不支持的账号从 CPA、工作区邮箱池和刷新队列一起移除。
- 新增长效手机接码池，支持手机号 + API URL、本地绑定账号、手动轮询验证码并记录状态。
- 开源版临时邮箱 Worker 默认值改成空配置和示例占位，避免把个人部署域名写进公开仓库。
- README 改为中文主体，删除中英文重复介绍。
- 去掉页面顶部的商城、中转站、公益站展示外链，开源版默认只保留 GitHub 入口。
- CPA 仓管巡检增加深度诊断：RT 失效、会话失效、封禁/停用、风控/地区受限、额度耗尽、非 OpenAI 凭证、探测失败等。
- 增加普通用户工作区隔离：服务端辅助数据按 `X-Workspace-Id` 写入 `data/workspaces/<workspace-id>/`。
- 修复中文文案乱码残留，并加入本地存储旧乱码值的前端修复。

## 2026-05-30

- 优化刷新工作台布局，右侧日志改成更紧凑的终端风格。
- 登录失败原因改成更清楚的中文分类，避免直接展示大段英文错误。
- sub2api 导出修正 `expires_at` 类型，避免导入时报 `int64` 解析错误。
- 成功账号、失败账号、需要接码账号在刷新池里分组展示。

## 2026-05-28

- 修复前端脚本语法问题，恢复邮箱刷新。
- 增加当前筛选结果的本地批量删除。
- 刷新失败时展示更具体的请求和错误原因。
- 压缩邮箱列表和邮件列表头部，让标题和计数更整齐。
- 隐藏笨重滚动条，同时保留列表滚动能力。
- 恢复 Microsoft IMAP HTML 邮件详情渲染。
- 增加浏览器本地备份和恢复。
- 导入 Outlook / 临时邮箱时增加预检摘要。
- 增加管理员登录页和 cookie 管理员会话。
- 设置 `MAIL_PICKUP_ADMIN_TOKEN` 后，自检页改为私有入口。
