...
def count_by_value(rows: list[Any], getter: Callable[[Any], str]) -> dict[str, int]:
    counts: dict[str, int] = {}
    for row in rows:
        key = str(getter(row) or "").strip().lower() or "unknown"
...
def dashboard_message_recipient(
...
def is_banned_mail_message(
    ...
    *,
    ...
    classify_mail: Callable[[str], str],
) -> bool:
...
def dashboard_stats_response(
    ...
    *,
    ...
    load_workspace_accounts: Callable[[str], dict[str, Any]],
    load_workspace_temp_addresses: Callable[[str], dict[str, Any]],
    load_workspace_generic_accounts: Callable[[str], dict[str, Any]],
    load_workspace_refresh_results: Callable[[str], list[dict[str, Any]]],
    load_workspace_messages: Callable[[str], list[dict[str, Any]]],
    parse_message_datetime: Callable[[Any], datetime | None],
    ...
    accounts = list(load_workspace_accounts(workspace_id).values())
    temp_addresses = list(load_workspace_temp_addresses(workspace_id).values())
    generic_accounts = list(load_workspace_generic_accounts(workspace_id).values())
    refresh_results = load_workspace_refresh_results(workspace_id)
    ...
    mailbox_error_count = sum(
        1
        for row in all_mailboxes
        if coerce_text(row.get("status")).lower() in {"error", "failed"} or coerce_text(row.get("error_code"))
    )
    ...
    for message in messages:
        if not is_banned_mail_message(message, coerce_text=coerce_text, classify_mail=classify_mail):
            continue
        parsed = parse_message_datetime(message.get("received_at") or message.get("cached_at"))
        ...
        recipient = dashboard_message_recipient(message, first_text=first_text)
    ...
    last_7_days_banned = sum(
        row["count"]
        for row in daily_rows
        if datetime.fromisoformat(row["date"]).date() >= seven_start
    )
    if banned_daily.get(today_key, 0) > 0:
        risk_level = "high"
        risk_reason = "today_banned_mail"
    elif last_7_days_banned > 0:
        risk_level = "warning"
    ...
    return {
        ...
        "banned_mail": {
            ...
            "last_7_days": last_7_days_banned,
