...
- `1.3.0` 主要新增工作区找回与备份恢复页：普通用户可以找回当前浏览器记住的工作区，管理员还能查看服务器工作区摘要，避免页面空白、按钮失效或切错工作区后直接丢数据。
- 现在可以直接导出 / 导入工作区备份，并支持可选密码加密；恢复时会带回账号、分组、队列和常用设置，但不会把管理员工具 token 一起导出。
- 新用户不再共享同一个硬编码默认工作区，系统会为新的浏览器实例生成自己的 workspace，减少多人共用 VPS 时的数据串扰。
- 静态资源版本号同步提升到 `1.3.0`，浏览器会重新拉取新的前端脚本。

- `1.1.1` 继续收紧凭证刷新页的前端结构，把最重的几块刷新交互正式拆成共享模块：来源列表与邮箱验证、手动邮箱码/手机码弹窗、长效手机池与绑定取码、本地刷新队列模型现在都从主文件里独立出来，后面继续维护和扩展会稳很多。
- 这次新增了 `refresh-source-list.js`、`refresh-manual-code.js`、`refresh-phone-pool.js` 和 `refresh-queue-model.js`，首页、邮箱管理页、刷新页三处本地入队逻辑也统一成一套，减少多个页面各写一套队列拼装逻辑带来的回归风险。
- 邮箱管理页补上了真正可用的“加入刷新队列”批量入口，现在可以直接按当前勾选邮箱推进刷新任务，不需要再绕别的页面操作。
- 登录任务 SQLite 持久化、活动统计 SQLite 查询和邮件缓存 SQLite sidecar 这些 `1.1.0` 的基础设施升级保持不变，当前版本是在这个基础上继续把刷新交互、队列状态和手动接码链路拆稳。
- 这一版重点仍然不是再堆新入口，而是把已有工作流拆清楚：来源筛选、邮箱验证、手动填码、长效手机、刷新队列、删除刷新项但保留邮箱库这些关键动作，现在边界更清楚，也更适合继续往下拆 `refresh-runner / job-polling / login-payload`。
- 当前整理后，回归测试仍然保持 `89 passed`，适合作为这一阶段的对外同步版本。
...
- `/recover.html`：工作区找回页。更新后页面空白、按钮失效、看不到原邮箱、或者浏览器切错工作区时，直接打开这个入口；普通用户只能恢复当前浏览器记住过的工作区，管理员登录后可以查看服务器工作区摘要。
