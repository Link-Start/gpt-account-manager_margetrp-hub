...
- 先拆 `server.py` 的数据隔离层：工作区 ID、工作区路径、JSON 读写和数据计数集中到 `storage/workspace.py`，后面继续处理邮箱库和刷新队列隔离会更稳。
- 把收信 provider 规则拆到 `mail_providers.py`：其他邮箱主机推断、收信模式、Microsoft provider 顺序、收信错误分类和并发调度都从主服务里抽出来。
- 新增 `refresh_state_machine.py`，刷新任务开始记录更细的阶段状态，比如检查邮箱、等待验证码、生成凭证、同步 CPA、成功和失败；前端原来的队列状态保持兼容。
- 这版主打收信稳定性、队列状态机和错误分类的底层整理，协议登录和 CPA 上传流程本身没有换路，避免一次拆太多影响线上刷新成功率。
