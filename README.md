...
- `1.2.0` 主要修复线上页面初始化问题：邮箱管理页会先完成邮箱账号模型初始化，再读取当前工作区邮箱；凭证刷新页会先准备长效手机池 helper，再恢复本地手机池和刷新队列。
- 这次更新不会改变已有邮箱、邮件缓存、刷新结果和工作区隔离逻辑，重点是让已经部署的数据稳定显示出来，避免页面明明有数据却因为前端脚本顺序报错显示为空。
- 静态资源版本号同步提升到 `1.2.0`，浏览器会重新拉取新的 `mailboxes.js` 和 `refresh.js`。
