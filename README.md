...
- `1.1.0` 并行完成了三条核心基础设施升级：A1 仪表盘/活动统计 SQLite 查询层，A2 登录任务状态 SQLite 持久化，A3 邮件缓存 SQLite 化。现在项目不再只是“开始接 SQLite”，而是已经把活动数据、任务状态和邮件缓存三块都推进到了可持续扩展的持久化轨道上。
- 活动数据方面：`refresh_results` 和 `login_history` 现在会优先读取对应的 `.sqlite3` sidecar，没有 sidecar 时再回退到原有 JSON；旧 JSON 第一次读取时会自动回填到 SQLite，不需要额外迁移步骤。
- 邮件缓存方面：服务端邮件缓存现在也有 SQLite sidecar，读取优先走 SQLite，旧 `messages.json` 首次读取会自动回填；对外前端接口和 `public` 工作区 root/workspace 双路径兼容保持不变。
- 登录任务方面：新增 SQLite 登录任务持久化，关键状态、日志、手动邮箱码/手机码和终止状态会落盘；服务重启后会恢复队列/历史视图，未完成任务统一标记为 `login_interrupted`，避免前端继续把中断任务当成正在运行。
